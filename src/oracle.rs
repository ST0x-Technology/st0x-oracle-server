...
/// turns the whole HTTP response into a 4xx/5xx and every item is lost.
/// With the flag the response is always `200`. Each request item maps to
/// exactly one of these, in request order. A caller keeps the items that
/// signed and sees which ones did not.
///
...
/// { "status": "error", "body": { "error": "no_live_quote", "detail": "…" } }
/// ```
///
/// The `error` body is the same `ErrorResponse` a non-2xx response
/// carries, built by `AppError::to_error_response`, so the `error` code
/// strings mean the same thing in both modes.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
mod batch_item_tests {
    ...
    fn error_item_serialises_with_status_error_and_error_body() {
        ...
        assert_eq!(
            json,
            r#"{"status":"error","body":{"error":"no_live_quote","detail":"No live quote for DRAM yet."}}"#
        );
    }
        ...
        let cases: Vec<(AppError, &str)> = vec![
            ...
            (
                AppError::Unavailable {
                    reason: crate::UnavailableReason::NoLiveQuote,
                    detail: "x".into(),
                },
                "no_live_quote",
            ),
