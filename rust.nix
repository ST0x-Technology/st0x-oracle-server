...
in
{
  # `nix build .#st0x-oracle-server` produces the binary that deploy-rs
  # ships to the droplet. `doCheck = false` to avoid running the full
  # test suite at deploy time — CI already gated it. Mirrors the
