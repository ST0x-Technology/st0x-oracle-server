...
async fn test_error_body_shape_is_stable_across_status_codes() {
    ...
    assert_eq!(body.error, "no_live_quote");
...
async fn test_batch_without_flag_keeps_all_or_nothing_behaviour() {
    ...
    for endpoint in ["/context/v1", "/context/v7"] {
        ...
        for query in non_flag_queries {
            ...
            let err: ErrorResponse = serde_json::from_value(body).unwrap();
            assert_eq!(err.error, "no_live_quote", "{uri}");
...
/// cache miss (no live quote → no_live_quote). The ok slots must
...
async fn test_v1_batch_with_flag_returns_per_item_envelope() {
    ...
    match &items[1] {
        BatchItemResponse::Error(e) => {
            assert_eq!(e.error, "no_live_quote");
            assert!(e.detail.contains("DRAM"), "{}", e.detail);
        }
        other => panic!("slot 1 should be no_live_quote, got {other:?}"),
    }
    match &items[2] {
    ...
    for slot in body.as_array().unwrap() {
        // Set equality: key ORDER in a parsed map depends on serde_json's
        // `preserve_order` feature, which only a transitive dependency
        // enables. The wire order is pinned by the string assertion in
        // `oracle::batch_item_tests`.
        let mut keys: Vec<_> = slot.as_object().unwrap().keys().collect();
        keys.sort();
        assert_eq!(keys, vec!["body", "status"], "{slot}");
    }
}
...
async fn test_v1_batch_with_flag_all_failed_is_200_with_error_slots() {
    ...
    let codes: Vec<&str> = items
        ...
        .collect();
    assert_eq!(codes, vec!["no_live_quote", "bad_request"]);
}
...
async fn test_pair_bound_batch_with_flag_returns_per_item_envelope() {
    ...
    for endpoint in PAIR_BOUND_ENDPOINTS {
        ...
        match &items[1] {
            BatchItemResponse::Error(e) => {
                assert_eq!(e.error, "no_live_quote", "{uri}");
                assert!(e.detail.contains("DRAM"), "{uri}: {}", e.detail);
            }
            other => panic!("{uri} slot 1 should be no_live_quote, got {other:?}"),
        }
        match &items[2] {
        ...
        for slot in body.as_array().unwrap() {
            let mut keys: Vec<_> = slot.as_object().unwrap().keys().collect();
            keys.sort();
            assert_eq!(keys, vec!["body", "status"], "{uri}: {slot}");
        }
    }
}
...
async fn test_pair_bound_all_failed_and_empty_batches_with_flag() {
    ...
    for endpoint in PAIR_BOUND_ENDPOINTS {
        ...
        let codes: Vec<&str> = items
            ...
            .collect();
        assert_eq!(codes, vec!["no_live_quote", "bad_request"], "{uri}");
...
async fn test_pair_bound_strict_resolve_error_wins_over_earlier_build_error() {
    ...
    for endpoint in PAIR_BOUND_ENDPOINTS {
        ...
        let items: Vec<BatchItemResponse> = serde_json::from_value(json).unwrap();
        assert!(
            matches!(&items[0], BatchItemResponse::Error(e) if e.error == "no_live_quote"),
            "{uri}"
        );
        assert!(
...
async fn test_v7_absent_underlying_rate_is_per_item_with_flag() {
    ...
    let pricing = LiveClient::with_seeded(vec![coin, dram], TEST_CHAIN_ID).await;
    let metrics = MetricsHandle::install().expect("metrics install");
